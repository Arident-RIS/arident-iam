...
public sealed class RolePermissionGrant : AuditableEntity
{
    ...
    internal static RolePermissionGrant Create(
        Guid roleVersionExternalId,
        ...
        string createdBy)
    {
        var entity = new RolePermissionGrant
        {
            ...
            GrantType = Guard.AgainstInvalidEnum(grantType, nameof(grantType)),
            ScopeType = Guard.AgainstInvalidEnum(scopeType, nameof(scopeType)),
    ...
    internal void UpdateConstraint(string? constraintJson, string updatedBy)
    {
        ConstraintJson = string.IsNullOrWhiteSpace(constraintJson) ? null : constraintJson.Trim();
        Touch(updatedBy);
    }
